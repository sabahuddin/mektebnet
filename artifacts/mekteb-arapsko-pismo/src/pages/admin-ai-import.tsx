...
export default function AdminAiImportPage() {
  ...
  const parsePreview = () => {
    ...
    try {
      ...
      if (!data.naslov || !data.slug || !Array.isArray(data.pitanja)) {
        setError("JSON mora sadržavati: naslov, slug, pitanja (niz)");
        return;
      }
      setPreview({
        naslov: data.naslov,
        ...
        kategorija: data.kategorija || "(nije postavljeno)",
      });
    } catch {
      setError("Nevalidan JSON format");
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError("Nevalidan JSON");
      return;
    }
    setSaving(true);
    try {
      const result = await apiRequest<any>("POST", "/admin/kvizovi/ai-import", data, token!);
      toast({ title: "Kviz importovan", description: `"${result.naslov}" — ${result.ukupnoPitanja} pitanja` });
      setJsonText("");
      setPreview(null);
      setLocation(`/admin/kviz/${result.kvizId}`);
    } catch (err: any) {
      toast({ title: "Greška", description: err?.message || "Import nije uspio", variant: "destructive" });
    } finally {
      setSaving(false);
    }
  };
    ...
    <Layout>
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="flex items-center gap-3 mb-6">
          ...
          <div>
            <h1 className="font-extrabold text-xl text-foreground">AI uvoz kviza</h1>
            <p className="text-sm text-muted-foreground">Nalepi JSON od Anthropic/Claude i klikni Import.</p>
          </div>
        </div>

        <div className="bg-white border border-border rounded-2xl p-5 shadow-sm space-y-4">
          <div>
            <label className="block text-sm font-semibold text-foreground mb-1">JSON kviza</label>
            <textarea
              value={jsonText}
          ...
          <div className="flex items-center gap-3">
            ...
            >
              ...
              {saving ? "Importujem..." : "Importuj kviz"}
            </button>
          </div>
        ...
        <div className="mt-6 bg-slate-50 border border-slate-200 rounded-xl p-4 text-sm text-slate-700">
          <p className="font-semibold mb-1">Kako koristiti:</p>
          <ol className="list-decimal list-inside space-y-1">
            <li>Pošalji Anthropic/Claude template (vidi ispod) da generiše kviz.</li>
            <li>Kopiraj JSON odgovor i nalepi ovdje.</li>
            <li>Klikni "Importuj kviz" — kviz i pitanja se automatski kreiraju.</li>
            <li>Ako pitanje već postoji u banci (isti tekst), koristi se postojeći ID.</li>
          </ol>
        </div>
      </div>
    </Layout>
