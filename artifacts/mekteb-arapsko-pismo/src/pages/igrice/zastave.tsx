      ...
      {state === "idle" && (
        <Card className="p-6 mb-6 bg-sky-50 border-sky-200">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <p className="font-bold text-foreground mb-1">{t("60 sekundi — koliko zastava prepoznaješ?")}</p>
              <p className="text-sm text-muted-foreground">{t("Pojavi se zastava — odaberi državu kojoj pripada. Zastave iz cijelog svijeta, sa naglaskom na muslimanske zemlje.")}</p>
      ...
      {state === "playing" && currentQ && (
        <>
          <div className="flex items-center justify-between gap-4 mb-4 text-sm font-bold">
            <span className="text-sky-600">{t("Odgovoreno:")} <span className="text-foreground" data-testid="text-answered-count">{answers.length}</span></span>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Card className="p-8 mt-6 bg-gradient-to-br from-sky-50 to-blue-50 border-sky-300 text-center">
              ...
              <p className="text-lg text-muted-foreground mb-2">
                {t("Rezultat:")} <span className="font-black text-3xl text-sky-600" data-testid="text-final-score">{finalScore}</span>
              </p>
                ...
                {bestEver !== null && (
                  <p>
                    ...
                    {previousBest !== null && finalScore !== null && finalScore > previousBest && (
                      <span className="ml-2 text-sky-600 font-bold">{t("novi rekord!")}</span>
                    )}
                  </p>
                )}
              ...
              <p className="text-sm text-muted-foreground mb-4">
                {t("Tačno:")} <strong className="text-sky-600" data-testid="text-correct-count">{finalScore}</strong> {t("· Netačno:")} <strong className="text-red-500" data-testid="text-wrong-count">{Math.max(0, answers.length - finalScore)}</strong>
              </p>
