  ...
  const lockAndAdvance = useCallback((p: Piece) => {
    ...
    if (clearedRows.length > 0) {
      ...
      if (clearedRows.length >= 2) {
        setComboMsg(clearedRows.length === 4 ? "🐝 BZZZ!" : clearedRows.length === 3 ? "Saća pucaju!" : "Bravo!");
    ...
    <Layout>
      <div className="flex items-center gap-3 mb-4 flex-wrap">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-foreground flex items-center gap-2">
          <Hexagon className="w-7 h-7 text-amber-500 fill-amber-200" /> Mektebsko saće
        </h1>
      </div>
      ...
      {state === "idle" && (
        <Card className="p-6 mb-6 bg-gradient-to-br from-amber-50 to-yellow-100/60 border-amber-200">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <p className="font-bold text-foreground mb-1">Slaži šestougaone ćelije saća kao u Tetrisu</p>
              <p className="text-sm text-muted-foreground leading-relaxed">
                Padajuće figure (1, 2, 3 ili 4 hex) se slažu u saće. Popuni cijeli horizontalni red i nestaje — dobiješ bodove. Igra traje dok ne izgubiš (ne stanu nove figure).
              </p>
              <p className="text-xs text-muted-foreground mt-2">
                <strong>Kontrole:</strong> ← → pomjeri, ↑ rotiraj, ↓ ubrzaj, Enter za hard drop. Na mobilnom: dugmad ispod.
              </p>
        ...
        <>
          ...
          <div className="grid grid-cols-4 gap-2 mb-3 text-center">
            <div className="bg-white border border-amber-200 rounded-2xl py-2 px-1">
              <p className="text-[10px] font-bold uppercase text-amber-700/70">Bodovi</p>
              <p className="text-lg font-black text-amber-700 tabular-nums" data-testid="text-score">{score}</p>
            </div>
            <div className="bg-white border border-amber-200 rounded-2xl py-2 px-1">
              <p className="text-[10px] font-bold uppercase text-amber-700/70">Level</p>
              <p className="text-lg font-black text-amber-700 tabular-nums" data-testid="text-level">{level}</p>
            </div>
            <div className="bg-white border border-amber-200 rounded-2xl py-2 px-1">
              <p className="text-[10px] font-bold uppercase text-amber-700/70">Linije</p>
            ...
            <div className="bg-white border border-amber-200 rounded-2xl py-2 px-1 flex flex-col items-center justify-center">
              <p className="text-[10px] font-bold uppercase text-amber-700/70 mb-0.5">Sljedeća</p>
        ...
        {state === "ended" && finalScore !== null && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <Card className="p-8 mt-6 bg-gradient-to-br from-amber-50 to-yellow-100/80 border-amber-300 text-center">
              ...
              <p className="text-2xl font-black text-foreground mb-1">Mašallah!</p>
              <p className="text-lg text-muted-foreground mb-2">
                Rezultat: <span className="font-black text-3xl text-amber-700" data-testid="text-final-score">{finalScore}</span>
              </p>
              ...
              <p className="text-sm text-muted-foreground mb-4">
                {linesCleared} {linesCleared === 1 ? "linija" : "linija"} obrisano · level {level}
              </p>
