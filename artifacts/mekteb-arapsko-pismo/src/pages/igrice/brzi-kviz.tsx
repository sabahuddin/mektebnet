...
export default function BrziKviz() {
  ...
  const startGame = useCallback(async () => {
    ...
    try {
      ...
      if (qs.length < 5) {
        setErrorMsg(t("Nema dovoljno pitanja u bazi."));
        setState("error");
        return;
      }
      ...
      else if (err.status === 503) { setErrorMsg(t("Nema dovoljno pitanja u bazi.")); setState("error"); }
    ...
    <Layout>
      <div className="flex items-center gap-3 mb-6 flex-wrap">
        ...
        <h1 className="text-2xl md:text-3xl font-black text-foreground flex items-center gap-2">
          <Zap className="w-7 h-7 text-orange-500" /> {t("Brzi kviz")}
        </h1>
      ...
      {state === "idle" && (
        <Card className="p-6 mb-6 bg-orange-50 border-orange-200">
          <div className="flex items-start gap-3 mb-4">
            ...
            <div>
              <p className="font-bold text-foreground mb-1">{t("60 sekundi — koliko tačnih?")}</p>
              <p className="text-sm text-muted-foreground">{t("Score = broj tačnih odgovora. Pitanja iz svih ilmihal lekcija. Pokušaj odgovoriti što više za 60 s — rezultat saznaješ na kraju.")}</p>
      ...
      {state === "playing" && currentQ && (
        <>
          ...
          <div className="flex items-center justify-between gap-4 mb-4 text-sm font-bold">
            <span className="text-orange-600">{t("Odgovoreno:")} <span className="text-foreground" data-testid="text-answered-count">{answers.length}</span></span>
