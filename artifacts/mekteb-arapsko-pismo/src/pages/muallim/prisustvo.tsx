      ...
      <div className="max-w-6xl mx-auto">
        ...
        <aside className="lg:sticky lg:top-24 rounded-2xl border border-border/50 bg-white/80 p-3">
          ...
          <div className="flex flex-col gap-1.5">
            <div className="flex items-center gap-2 rounded-xl border border-primary/30 bg-primary/10 px-3 py-2.5 text-sm font-bold text-primary"><CalendarCheck className="h-4 w-4 shrink-0" />{t("Prisustvo")}</div>
            {grupa && (<>
              <button type="button" onClick={() => setLocation(`/muallim/grupa/${grupa.id}?modul=plan`)} className="flex w-full items-center gap-2 rounded-xl border border-border/60 bg-white px-3 py-2.5 text-left text-sm font-bold text-foreground transition-colors hover:border-primary/40 hover:bg-primary/5"><CalendarCheck className="h-4 w-4 shrink-0 text-muted-foreground" />{t("Plan lekcija")}</button>
              <button type="button" onClick={() => setLocation(`/muallim/grupa/${grupa.id}`)} className="flex w-full items-center gap-2 rounded-xl border border-border/60 bg-white px-3 py-2.5 text-left text-sm font-bold text-foreground transition-colors hover:border-primary/40 hover:bg-primary/5"><ArrowLeft className="h-4 w-4 shrink-0 text-muted-foreground" />{t("Nazad u grupu")}</button>
            </>)}
          </div>
        </aside>
        </div>
