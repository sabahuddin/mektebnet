...
Postojeći kvizovi Nivoa 1 i Nivoa 2 razvrstani su u provjere znanja vezane za
blokove od po deset lekcija, i u završne kvizove nivoa.

## Struktura

Svaki nivo ima **sedam etapa**, u skladu sa `kvizovi.etapa` (1 = lekcije 1–10,
2 = 11–20, … 7 = 61 i dalje). Etapni kviz se veže za medaljon-lekciju
`medaljon-nivo{N}-{etapa}` preko `kvizovi.lekcija_id`.

| Nivo | Lekcija | Etapa 7 pokriva | Kvizovi |
|------|---------|-----------------|---------|
| 1 | 62 u seedu (`redoslijed` 0–63, nedostaju 10, 22, 23, 55) | lekcije 61–64 | `1-etapa-1`…`1-etapa-7`, `1-krunisanje-a/b/c` |
| 2 | 68 (`redoslijed` 0–67, nedostaje 29) + uvodna na −10 | lekcije 61–68 | `2-etapa-1`…`2-etapa-7`, `2-krunisanje-a/b/c` |
...
lekcije (Zikr, Sura En-Nasr, Bajramske aktivnosti, Sport).

## Odakle pitanja

| Izvor | Nivo 1 | Nivo 2 |
|-------|--------|--------|
| Postojeći kvizovi | 572 pitanja, 530 jedinstvenih | 1093 pitanja, 989 jedinstvenih |
| Ugrađeni kvizovi lekcija | 300 | 348 |
| Novonapisana pitanja | 82 | 58 |

Svako pitanje iz postojećih kvizova ručno je razvrstano u blok lekcija kojem
sadržajno pripada — mape su u `scripts/data/nivo{1,2}-mapa-pitanja.json`.
Vrijednost `0` u mapi Nivoa 2 označava pitanja koja ponavljaju gradivo Nivoa 1;
ona ne ulaze ni u jednu etapu, nego samo u bazen krunisanja.
...
u seedu prazne: `tesbih` (Nivo 1) i `uvodna-rijec-nivo-2` (Nivo 2).
...
- `scripts/src/nivo1-etape-lib.test.ts` — testovi logike i provjera stvarnih podataka za oba nivoa.
...
- `scripts/data/nivo{1,2}-mapa-pitanja.json` — pitanje → blok lekcija.
- `scripts/data/nivo1-lekcijska-ispravke.json`, `scripts/data/nivo2-kviz-ispravke.json` — ispravke izvora.
- `scripts/data/nivo{1,2}-nova-pitanja.json` — novonapisana pitanja.
