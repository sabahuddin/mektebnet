...
- **Admin Čitaonica CRUD (maj 2026):** Admin može kroz UI (`/admin/citaonica`, link u admin nav baru) kreirati/uređivati/brisati priče u Čitaonici. Backend rute: `GET/POST/PUT/DELETE /api/admin/knjige(/:id)` u `admin.ts` (admin-only guard). Public `/api/content/knjige` i `/api/content/knjige/:slug` filtriraju po `isPublished = true` (neobjavljene priče vraćaju 404 i nisu vidljive na javnoj /citaonica). Forma koristi WysiwygEditor (TipTap) za sadržaj — admin može ubacivati slike preko postojećeg `/api/admin/upload` (multer + sharp → webp 82, max 1600px), iste galerije kao za ilmihal lekcije. Cover slika se uploaduje istim endpointom. Auto-slug generacija iz naslova sa toggle za ručno editovanje. Polja u modalu: naslov, slug, kategorija (`prica` ili `ostalo`), redoslijed, isPublished. Slug ima unique constraint → 409 sa porukom "Slug već postoji".
