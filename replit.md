...
- `POST /start { gameId: "memory"|"quiz" }` — **role-guard `ucenik`**. Kreira `game_sessions` red, vraća sessionId, startedAt, allowedDurationSec. Trajanje runde cap-uje server: quiz=60s (ROUND_DURATION_SEC), memory bez fiksnog cap-a (cap = preostali credit, ali ≤30min). Atomski guard kroz partial unique index `(user_id) WHERE status='running'` — duplicate start vraća 409. Auto-expire stare running sesije clamp-uje `duration_sec = LEAST(allowed_duration_sec, NOW()-started_at)` (ne fiksno 1800).
- `POST /end { sessionId, score }` — **role-guard `ucenik`**. Atomski UPDATE sa `WHERE status='running'`. Server računa duration_sec iz NOW()-startedAt. Score se clamp-uje na MAX (memory=1000, quiz=60) i sanity-cap (cheatCap = maxScore × elapsedSec/minSec; minSec memory=5, quiz=15) — instant submit max score-a vraća 0. Vraća `{ ok, score, finalScore, durationSec }`.
